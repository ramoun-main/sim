--- ### Cosmic Updater
--
-- Cosmic Updater utilities to use within Cosmic and user configurations.
--
-- This module can also loaded with `local updater = require("cosmic.utils.updater")`
--
-- @module cosmic.utils.updater
-- @see cosmic.utils
-- @copyright 2022
-- @license GNU General Public License v3.0

local git = require "cosmic.utils.git"
...
--- Helper function to generate Cosmic snapshots (For internal use only)
...
function M.generate_snapshot(write)
  local file
  local prev_snapshot = require(cosmic.updater.snapshot.module)
  ...
  if write == true then
    file = assert(io.open(cosmic.updater.snapshot.path, "w"))
    file:write "return {\n"
  end
...
--- Get the current Cosmic version
---@param quiet? boolean Whether to quietly execute or send a notification
---@return string # The current Cosmic version string
function M.version(quiet)
  local version = cosmic.install.version or git.current_version(false) or "unknown"
  if cosmic.updater.options.channel ~= "stable" then version = ("nightly (%s)"):format(version) end
  if version and not quiet then notify("Version: " .. version) end
  return version
end

--- Get the full Cosmic changelog
---@param quiet? boolean Whether to quietly execute or display the changelog
---@return table # The current Cosmic changelog table of commit messages
...
function M.update_packages()
  ...
  require("cosmic.utils.mason").update_all()
end

--- Create a table of options for the currently installed Cosmic version
...
function M.create_rollback(write)
  ...
  if write == true then
    local file = assert(io.open(cosmic.updater.rollback_file, "w"))
...
--- Cosmic's rollback to saved previous version function
function M.rollback()
  local rollback_avail, rollback_opts = pcall(dofile, cosmic.updater.rollback_file)
...
function M.update_available(opts)
  if not opts then opts = cosmic.updater.options end
  opts = require("cosmic.utils").extend_tbl({ remote = "origin" }, opts)
...
--- Cosmic's updater function
---@param opts? table the settings to use for the update
function M.update(opts)
  if not opts then opts = cosmic.updater.options end
  opts = require("cosmic.utils").extend_tbl(
  ...
  then
    ...
    local summary = {
      { "Cosmic updated successfully to ", "Title" },
      { git.current_version(), "String" },
      { "!\n", "Title" },
      {
        opts.auto_quit and "Cosmic will now update plugins and quit.\n\n"
    ...
    -- if the user wants to auto quit, create an autocommand to quit Cosmic on the update completing
    if opts.auto_quit then
      vim.api.nvim_create_autocmd("User", {
        desc = "Auto quit Cosmic after update completes",
