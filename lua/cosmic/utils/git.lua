--- ### Git LUA API
--
-- This module can be loaded with `local git = require "cosmic.utils.git"`
--
-- @module cosmic.utils.git
-- @copyright 2022
...
--- Run a git command from the Cosmic installation directory
---@param args string the git arguments
---@return string|nil # The result of the command or nil if unsuccessful
function git.cmd(args, ...)
  return require("cosmic.utils").cmd("git -C " .. cosmic.install.home .. " " .. args, ...)
end

--- Check if the Cosmic is able to reach the `git` command
---@return boolean # The result of running `git --help`
function git.available() return vim.fn.executable "git" == 1 end

--- Check if the Cosmic home is a git repo
...
---@return string|nil # The branch of the Cosmic installation
...
function git.parse_remote_url(str)
  return vim.fn.match(str, require("cosmic.utils").url_matcher) == -1
      and git.url .. str .. (vim.fn.match(str, "/") == -1 and "/Cosmic.git" or ".git")
    or str
end
...
---@param commits string[] an array like table of commit messages
---@return string[][] # An array like table of echo messages to provide to nvim_echo or cosmic.echo
