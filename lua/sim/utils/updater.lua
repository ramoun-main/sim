--- ### Editor Updater
--
-- Editor Updater utilities to use within Editor and user configurations.
--
...
local git = require(string.format('%s.utils.git', editor_name))
...
--- Helper function to generate Editor snapshots (For internal use only)
...
--- Get the current Editor's version
...
function M.version(quiet)
  ...
  if cosmic.updater.options.channel ~= 'stable' then
...
--- Get the full Editor changelog
---@param quiet? boolean Whether to quietly execute or display the changelog
---@return table # The current changelog table of commit messages
...
function M.update_packages()
  ...
  require(string.format('%s.utils.mason', editor_name)).update_all()
end

--- Create a table of options for the currently installed Editor version
...
--- Editor's rollback to saved previous version function
function M.rollback()
...
function M.update_available(opts)
  if not opts then
    opts = cosmic.updater.options
  end
  opts = require(string.format('%s.utils', editor_name)).extend_tbl({ remote = 'origin' }, opts)
...
--- Editor's updater function
---@param opts? table the settings to use for the update
function M.update(opts)
  if not opts then
    opts = cosmic.updater.options
  end
  opts = require(string.format('%s.utils', editor_name)).extend_tbl(
  ...
  then
    ...
    local summary = {
      { string.format('The %s Editor updated successfully to ', editor_name), 'Title' },
      { git.current_version(), 'String' },
      { '!\n', 'Title' },
      {
        opts.auto_quit and string.format('The %s Editor will now update plugins and quit.\n\n', editor_name)
    ...
    -- if the user wants to auto quit, create an autocommand to quit Editor on the update completing
    if opts.auto_quit then
      vim.api.nvim_create_autocmd('User', {
        desc = string.format('Auto quit %s Editor after update completes', editor_name),
