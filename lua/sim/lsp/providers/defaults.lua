...
local M = {}
local augroup_name = string.format('%sLspFormat', editor_name)
...
function M.on_attach(client, bufnr)
  ...
  if client.supports_method('textDocument/formatting') then
    ...
    vim.cmd(string.format("command! LspFormat lua require('%s.utils.lsp').format(%s)", editor_name, bufnr))
    ...
    if user_config.lsp.format_on_save then
      ...
      vim.api.nvim_create_autocmd(string.format('BufWritePre %s', filetype_pattern), {
        callback = function()
          require(string.format('%s.utils.lsp', editor_name)).format(bufnr)
        end,
        buffer = bufnr,
        group = group,
      })
  ...
  require(string.format('%s.lsp.mappings', editor_name)).init(client, bufnr)
...
M.root_dir = function(fname)
  ...
  return util.root_pattern('.git')(fname)
    ...
    or util.root_pattern('.eslintrc.cjs')(fname)
    or util.root_pattern('.eslintrc.json')(fname)
    or util.root_pattern('tsconfig.json')(fname)
    or util.root_pattern('tsconfig.node.json')(fname)
end
